  ...
  const handleRegister = async (e) => {
    ...
    try {
      await register(
        ...
        formData.password, 
        formData.role,
        formData.ipsId ? parseInt(formData.ipsId) : null
      );
      alert('Registro exitoso. Ahora puedes iniciar sesión.');
      setActiveTab('login');
      setFormData({ name: '', email: '', password: '', role: '', ipsId: '' });
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
          ...
          <div className="animate-enter">
            {activeTab === 'login' ? (
              ...
              <form onSubmit={handleRegister} className="space-y-6">
                ...
                {/* Campo IPS - Solo visible para administradores */}
                {formData.role === 'administrador' && (
                  <div className="animate-slide-down">
                    <label className="block text-sm font-semibold text-medical-700 mb-2">
                      IPS (Institución Prestadora de Salud) <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg className="h-5 w-5 text-medical-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <p className="mt-2 text-xs text-medical-500">
                      Selecciona la IPS a la que perteneces como administrador
                    </p>
                  </div>
                )}
